...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-end-activity').addEventListener('click', endActivity);
...
function renderTrack(trackId, students, currentQuestionIndex, showStatus) {
  ...
  keys.forEach((key, i) => {
    ...
    const pos = clampPosition(student.position || 0, -maxRange, maxRange);
    const xPct = n <= 1 ? 50 : (8 + (i / (n - 1)) * 84);
    const yPct = 50 - (pos / maxRange) * 42;
