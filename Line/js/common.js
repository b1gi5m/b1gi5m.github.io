...
const DEFAULT_QUESTIONS = [
  { text: "어쩔 수 없이 1~2년 단위로 이사를 다녀야 한다면", forwardLabel: "해당 없음", backLabel: "해당함 → 뒤로", forwardDelta: 0, backDelta: -1 },
  { text: "4대 보험을 받지 못하는 일을 하고 있는 가정이라면", forwardLabel: "해당 없음", backLabel: "해당함 → 뒤로", forwardDelta: 0, backDelta: -1 },
  { text: "가족 형태 때문에 학교나 또래 집단에서 놀림이나 곤란한 일을 겪은 적이 있다면", forwardLabel: "해당 없음", backLabel: "해당함 → 뒤로", forwardDelta: 0, backDelta: -1 },
  { text: "매 학기 원하는 학원이나 방과후 활동을 자유롭게 선택할 수 있다면", forwardLabel: "해당함 → 앞으로", backLabel: "해당 없음", forwardDelta: 1, backDelta: 0 }
];
  ...
  if (qSheetName) {
    ...
    rows
      .map(row => ({
        ...
        backLabel: String(row["뒤로버튼텍스트"] || row["back_label"] || "해당함 → 뒤로").trim(),
